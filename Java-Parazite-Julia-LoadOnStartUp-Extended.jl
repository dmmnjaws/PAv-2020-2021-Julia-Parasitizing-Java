...
function generateSuperTypes(foo)
    ...
    getAllSuperclasses = first(listmethods(Helper))
    returnType = eval(convertPrimitive(JavaCall.jimport(getname(getreturntype(getAllSuperclasses)))))
    parameterTypes = getparametertypes(getAllSuperclasses)
    parameterTypes = tuple(map(x -> eval(convertPrimitive(JavaCall.jimport(getname(x)))), parameterTypes)...)
    allSuperclasses = jcall(Helper, "getAllSuperclasses", returnType, parameterTypes, class)
    allSuperclasses = map(x -> eval(importExprBuilder(getname(x))), allSuperclasses)
end
